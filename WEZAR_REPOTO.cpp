...
int menu()
{
	...
	while (true) {
		...
		for (int i = 0; i < size(menuSTR); i++) {
			if (i == activeThemeMenu) SetConsoleTextAttribute(hstdout, FOREGROUND_GREEN | FOREGROUND_INTENSITY); //смена цвета
			else SetConsoleTextAttribute(hstdout, FOREGROUND_GREEN | FOREGROUND_BLUE | FOREGROUND_RED);
...
int main()
{
	...
	while (_getch() != ESC) {
		...
		if (menuN == 1) {
			...
			cout << "Средняя температура  =  " << sr << endl << "Нажмите любую кнопку для возвращения в главное меню";
			...
			Find_Gisss = str_response.find("_c", Find_Giss + 13);//unit unit_temperature_c
			//cout << Find_Gisss << endl << endl;
			...
			if (str_response[Find_Gisss + 4] == '&') {
				...
				cout << str_response[Find_Gisss + 4] << str_response[Find_Gisss + 5];
				res_gis = res_gis + str_response[Find_Gisss + 4] + str_response[Find_Gisss + 5];
			}
			cout << endl << res_gis;
			cout << endl << endl << endl << endl << endl;
